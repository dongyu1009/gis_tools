...
int main()
{
    ...
    {
        char filename[100];
        sprintf(filename, "MOSAIC_TMP_%d001.tif", year);
    ...
    for(int i = 0; i < xsize * ysize; i++)
    {
        ...
        if (values[0] != nodatavalue && values[0] < 32000){
            n3stdgt0[i] = ((mean - 3 * std) < 0) ? 0 : (mean - 3 * std) / 10;
            n2stdgt0[i] = ((mean - 2 * std) < 0) ? 0 : (mean - 2 * std) / 10;
            nstdgt0[i] = ((mean - std) < 0) ? 0 : (mean - std) / 10;
            n3std[i] = (mean - 3 * std) / 10;
            n2std[i] = (mean - 2 * std) / 10;
            nstd[i] = (mean -  std) / 10;
            pstd[i] = std;
        } else {
